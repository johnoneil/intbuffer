...
//==============================================================================
${type._name} ${type._name}::Parse(const EDC::IGameEvent& gameEvent)
{
  Int32 index = 0;
  return ${type._name}::Parse(gameEvent, index);
}

//==============================================================================
//Static method that returns instance of class from game event starting at index
//==============================================================================
${type._name} ${type._name}::Parse(const EDC::IGameEvent& gameEvent, Int32& index)
{
  ...
  % endif
	% for child in type._children:
    % if child.__class__.__name__ == 'Integer':
  returnValue.m_${child._name}=gameEvent.GetParam(index++);
      %if child._default >=0:
    ...
    % elif child.__class__.__name__ == 'Repeated':
  {
    ...
    for(Int32 i=0;i<count;++i)
    {
     ...
     Int32 value = gameEvent.GetParam(index++);
      % else:
      ${child._element._name} value= ${child._element._name}::Parse(gameEvent, index);
      % endif
    ...
    % elif child.__class__.__name__ == 'Set':
  {
    ...
    for(Int32 i=0;i<count;++i)
    {
	  ...
	  % else:
  returnValue.m_${child._name} = ${child._name}::Parse(gameEvent, index);
	  % endif
	% endfor
  return returnValue;
}
...
//==============================================================================
bool ${settings['namespace']}::Is${type._name}(const EDC::IGameEvent& gameEvent)
{
  try
  {
    ${type._name} value = ${type._name}::Parse(gameEvent);
  }catch(...)
  {
    return false;
  }
